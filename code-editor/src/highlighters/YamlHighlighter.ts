namespace CodeEditor.Highlighters {
     ...
     */
    export class YamlHighlighter implements ILanguageHighlighter {
        ...
        private tokenizeValue(text: string, b: TokenBuilder, state: any): void {
            let i = 0;
            const n = text.length;
            ...
            for (let k = 0; k < n; k++) {
                const c = text[k];
                if (inStr) {
                    if (c === inStr && text[k - 1] !== "\\") inStr = null;
                    continue;
                }
                if (c === '"' || c === "'") {
                    inStr = c;
        ...
        private tokenizeInlineValue(text: string, b: TokenBuilder): void {
            let i = 0;
            const n = text.length;
